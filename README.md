...
- 좌석배치도는 홍익대 대학로아트센터 대극장 배치(1층 OP·A·B·C, 2층 A·B·C)를 따릅니다. 존재하지 않는 좌석은 입력을 마치는 순간(엔터/포커스 이동) 삭제되어 집계되지 않습니다. 좌석을 기록하지 않고 관람만 집계하려면 `0000`을 입력하세요 (좌석배치도에는 표시되지 않음).
